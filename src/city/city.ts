...
function onStilts(pt: P5.Vector, dx: number, dz: number, h: number, color?: P5.Color | string): void {
    let nrx: number = int((dx - 1) / 6), nrz: number = int((dz - 1) / 6),
...
function base(pt: P5.Vector, h: number, color?: P5.Color | string) {
    let nr1: number = int(rdm(1, 5)), nr2: number = int(rdm(1, 5));
    let inter1: number = 27.0 / nr1, inter2: number = 27.0 / nr2;
...
function cubeBuilding(pt: P5.Vector): void {
    let h: number = int(rdm(50, 100));
    let c1: P5.Color = cp.colors.get(Element.STRUCTURE), c2: P5.Color = cp.colors.get(Element.STRUCTURE);
    const core: Function = () => {
    ...
    if (rdm() < .2) {onStilts(pt, 8, 15, 12, cp.colors.get(Element.STROKE));core();}
    else {core();base(pt, 12, c1);}
}
