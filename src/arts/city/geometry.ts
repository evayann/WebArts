...
import {
    Drawer, Action, Colors, Element,
    action, fill, stroke, resetColor, rdm
} from './drawer';
 ...
 */
const distance = 90;
const alpha: number = 30 * (Math.PI / 180);
const cAlpha: number = Math.cos(alpha);
const sAlpha: number = Math.sin(alpha);
...
export class CityParameters {
    ...
    constructor(w: number, h: number, c: Colors = null, o = 3,
                sl = .9, ip = .67, go = .05) {
        this.width = w;
        this.height = h;
        this.colors = c;
...
export function setGeometryParameters(p: p5Instance, c: CityParameters): void {
    p5 = p;
    cp = c;
}
 ...
 */
const off = (): number => rdm(-cp.offset, cp.offset);
const int = (x): number => p5.floor(x);
 ...
 */
export function projection(x: number, z: number, y = 0, debug = false): [number, number] {
    const v: P5.Vector = projectionVector(vec(x, z, y), debug);
    return [v.x, v.y];
}
 ...
 * @param debug print value of projection in console
 */
export function projectionGetV(x: number, z: number, y = 0, debug = false): P5.Vector {
...
export function projectionVector(pt: P5.Vector, debug?: boolean): P5.Vector {
    const pj: P5.Vector = projectionFactor(pt);
    const scale: number = cp.height / 90;
    const v = vec(cp.intersectPos * cp.width + scale * pj.x, 0, (cp.skyLimit + cp.groundOff) * cp.height + scale * pj.y);
...
export function projectionFactor(pt: P5.Vector): P5.Vector {
    const camHeight = 1.5;
    const a: number = pt.x * sAlpha - pt.z * cAlpha;
    const b: number = pt.x * cAlpha + pt.z * sAlpha;
 ...
 * @param y the Z axis
 */
export function vec(x: number, z: number, y = 0): P5.Vector {
 ...
 */
export function ellipse(cx: number, cy: number, rx: number, ry: number, fillColor: P5.Color, nbLoop?: number,
                        border = true, fct?: (number) => number | undefined): void {
    if (border) stroke(cp.colors.get(Element.STROKE));
    if (fillColor) fill(fillColor);

    p5.beginShape();
    const r: number = rdm(2.1, 2.5);
    let v: number;
    const f = (fct === undefined) ? (x) => 2.5 * cp.offset * p5.sin(r * x) : fct;
    const min: number = p5.min(rx, ry);
    const to: number = isNaN(nbLoop) ? p5.TAU * rdm([2, 3, 4]) : p5.TAU * nbLoop;
...
export function computeArc(pt: P5.Vector, from: number, to: number,
                           rx: number, ry: number, precision = 25): Array<P5.Vector> {
    const formula = (i): P5.Vector => projectionGetV(pt.x + p5.cos(i) * rx, pt.z - p5.sin(i) * ry, pt.y);
 ...
 * @param border if we have ellipse stroke
 */
export function circle(cx: number, cy: number, r: number, fillColor: P5.Color, nbLoop?: number, border = true): void {
 ...
 */
export function rect(sx: number, sy: number, ex: number, ey: number, fillColor: P5.Color, fillDirection?: Direction, border = false): void {
 ...
 */
export function polygon(vertices: P5.Vector[], fillColor?: P5.Color | string, border = true): void {
    if (fillColor) fill(fillColor);
...
export function box(p1: P5.Vector, p2: P5.Vector, boxColor?: P5.Color | string, underBoxColor?: P5.Color | string): void {
    const c1: P5.Vector = projectionGetV(p1.x, p2.z, p1.y),
...
export function cylinder(pt: P5.Vector, h: number, r: number, precision?: number,
                         color?: P5.Color | string, border = true, shadow = 3) {
    const bottoms: Array<P5.Vector> = computeArc(P5.Vector.add(pt, vec(15, 15)), 0, p5.TAU, r, r, precision);
    const tops: Array<P5.Vector> = computeArc(P5.Vector.add(pt, vec(15, 15, h)), 0, p5.TAU, r, r, precision);

    const zip = (x, y) => x.map((vx, i) => [vx, y[i]]);
    const pts: Array<[P5.Vector, P5.Vector]> = zip(tops, bottoms);
    let [prevTop, prevBottom] = pts[pts.length - 1];
    let shCounter = 0;

    add(action(polygon, tops, color, border));
    for (const [ti, bi] of pts) {
 ...
 */
export function strip(distToDo: number, p1: P5.Vector, p2: P5.Vector, p3: P5.Vector, p4: P5.Vector, stripOff = 6): void {
    const r1 = (p1.y - p2.y) / distToDo;
    const r2 = (p3.y - p4.y) / distToDo;
    let pp = 0;
    while (pp < distToDo) {
